  ...
  try {
    ...
    if (pathname === '/helloworld' && request.method === 'GET') {
      try {
        ...
        const message = await callGrpcService('demo2-service-24386434020.asia-northeast1.run.app.internal:50051', 'World')
        console.log(message)
    ...
    if (pathname === '/go' && request.method === 'GET') {
      try {
        ...
        const document = await fetchDocument('demo2-service-24386434020.asia-northeast1.run.app.internal')
        response.writeHead(200, { 'Content-Type': 'application/json' })
